...
import { findAllClientIds, getActiveClientId, debugClientStructure } from "./findClientIds.js";
...
const CapsolverResponseSchema = z.object({
  errorId: z.number().int().describe("Error message: 0 - no error, 1 - with error"),
  ...
  status: z.enum(["idle", "processing", "ready"]).describe("Task status: idle - Waiting, processing - Under identification, ready - The identification is complete"),
  solution: z.object({
    userAgent: z.string(),
    gRecaptchaResponse: z.string()
  }).optional().describe("Task result data containing userAgent and gRecaptchaResponse")
});
...
const capsolverResponse = await client.post('https://api.capsolver.com/createTask', {
...
while (attempts < maxAttempts) {
  taskResultResponse = await client.post('https://api.capsolver.com/getTaskResult', {
    clientKey: CAPSOLVER_KEY,
  ...
  if (result.status === 'ready') {
    if (result.errorId === 1) {
      throw new Error(`Capsolver task failed: ${result.errorDescription || result.errorCode || 'Unknown error'}`);
    } else if (result.errorId === 0) {
      finalResult = result;
      console.log(`Task ${capsolverResponse.data.taskId} completed successfully`);
      break;
    }
  }
  
  console.log(`Task status: ${result.status}, attempt ${attempts + 1}/${maxAttempts}`);
  attempts++;
...
if (attempts >= maxAttempts) {
  throw new Error(`Task ${capsolverResponse.data.taskId} timed out after ${maxAttempts} attempts`);
}
...
const injectionSuccess = await injectTokenIntoGrecaptcha(page, finalResult.solution.gRecaptchaResponse);
console.log('Token injection successful:', injectionSuccess);

await page.evaluate(({ token, clientId }: { token: string, clientId: string }) => {
